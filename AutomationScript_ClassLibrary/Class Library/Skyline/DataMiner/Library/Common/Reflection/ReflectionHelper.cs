	...
	internal static class ReflectionHelper
	{
		...
		private static List<Assembly> Load(string clpAssembly)
		{
			...
			foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
			{
				try
				{
					AddToLoadedFiltered(clpAssembly, loadedAssemblies, assembly);
				}
				catch (Exception e)
				{
					Logger.Log("Reflection Error while Loading Assemblies. Could not handle logic for: " + assembly.FullName + " with exception:" + e);
				}
			}
		...
		private static void AddToLoadedFiltered(string clpAssembly, List<Assembly> loadedAssemblies, Assembly assembly)
		{
